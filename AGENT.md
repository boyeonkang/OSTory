...
단, 개발은 기능별로 단계적으로 진행하며, 각 기능은 TMDB API, Gemini API, 앱 내부 저장 구조를 조합하여 구현한다.
...
* 감상 기록은 앱 내부 저장 구조에 저장한다.
* 시간이 부족한 경우 초기 구현은 메모리 기반 저장소로 시작하되, 이후 DataStore 또는 Room으로 확장 가능한 구조로 작성한다.
...
* 각 음악 정보에는 곡명, 가수, 작곡가, 작사가, 원곡자, 앨범 또는 발매 정보를 표시한다.
...
* AI 취향 분석 화면은 `Design/08-ai-analysis.png`를 기준으로 구현한다.
...
Gemini 응답이 실패하거나 비어 있는 경우 기본 안내 문구를 표시한다.
...
* 각 OST 정보에는 곡명, 가수, 작곡가, 작사가, 원곡자, 앨범 정보를 포함한다.
